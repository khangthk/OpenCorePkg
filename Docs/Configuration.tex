...
\item
  ...
  This keyboard combination requires \texttt{AllowNvramReset} to be set to \texttt{true}.
  Another way to erase UEFI variables is to choose the \texttt{Reset NVRAM} in the OpenCore picker,
  which is an auxiliary entry added by loading the \hyperref[uefiresetnvram]{\texttt{ResetNvramEntry}} driver.
...
\item
  ...
  \textbf{Type}: \texttt{plist\ boolean}\\
  \textbf{Failsafe}: \texttt{false}\\
  \textbf{Description}: Allow \texttt{CMD+OPT+P+R} handling.
  ...
  \emph{Note 2}: Resetting NVRAM using this keyboard combination will also erase any boot
  options not backed up using the bless command. For example, Linux installations to custom
  locations not specified in BlessOverride. To obtain reset NVRAM functionality which
  does not remove boot options, use the Reset NVRAM picker entry from the \hyperref[uefiresetnvram]{\texttt{ResetNvramEntry}}
  driver with the \texttt{-{}-preserve-boot} option specified.
...
Adds a menu entry which resets NVRAM and immediately restarts. The \texttt{AllowNvramReset} option
to allow resetting NVRAM using \texttt{CMD+OPT+P+R} is separate, and they can be enabled independently.

See the warning about resetting NVRAM on Lenovo laptops given at the \texttt{AllowNvramReset} setting.
...
\begin{itemize}
  ...
  \item \texttt{-{}-native} - Boolean flag, enabled if present. \medskip

  On Apple firmware only, this performs a native NVRAM reset. This can result in
  ...
  although note that only one boot chime will be heard. \medskip

  \emph{Note 1}: Due to using native NVRAM reset, this option is not compatible with the \texttt{-{}-preserve-boot} option
  and will override it, therefore all BIOS boot entries will be removed. \medskip

  \emph{Note 2}: Due to using native NVRAM reset, the OpenCore boot option
