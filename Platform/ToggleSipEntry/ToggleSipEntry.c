...
STATIC OC_PICKER_ENTRY  mToggleSipBootEntries[1] = {
  {
    ...
    .TextMode      = FALSE,
    .RealPath      = FALSE,
    .SystemAction  = InternalSystemActionSetSip,
    .ActionConfig  = NULL, ///< in this driver we use static variables, instead
  ...
  )
{
  ...
  //
  // Is CHAR_NULL terminated string present?
  //
  if (  (LoadedImage->LoadOptions != NULL)
     && ((LoadedImage->LoadOptionsSize % 2) == 0)
     && ((LoadedImage->LoadOptionsSize / 2) > 0)
     && (((CHAR16 *)LoadedImage->LoadOptions)[(LoadedImage->LoadOptionsSize / 2) - 1] == CHAR_NULL)
        )
  {
